...
import { parseWorkTime, formatMinutes, getStandardWorkingMinutes, getCumulativeActualMinutes, getElapsedWorkingMinutes } from '@/lib/time-utils';
...
export default function Dashboard() {
  ...
  const breakOptions = Array.from({ length: 13 }, (_, i) => i * 5); // 0,5,10,...,60
  ...
  const stats = useMemo(() => {
    ...
    // Build a map of logged dates for fast lookup
    const logMap = new Map(currentMonthLogs.map(log => [log.date, log.actualMinutes]));
    const loggedDates = new Set(logMap.keys());

    // Actual = logged days actual + unlogged working days × 8h (up to today)
    const actualMins = getCumulativeActualMinutes(
      year, month, loggedDates, (dateStr) => logMap.get(dateStr) ?? 0
    );

    // Surplus = actualMins so far - elapsed standard mins (only days up to today)
    const elapsedMins = getElapsedWorkingMinutes(year, month);
    const surplusMins = actualMins - elapsedMins;

    return { standardMins, actualMins, surplusMins };
  ...
  const handleEditLog = (log: any) => {
    ...
    setBreakMinutes(log.breakMinutes ?? 60);
  };
  ...
  const formatBreakLabel = (mins: number) => {
          ...
          <form onSubmit={handleInputSubmit} className="space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3 max-w-4xl">
              ...
              <div className="sm:w-36">
                <label className="text-sm text-muted-foreground mb-2 block">休憩時間</label>
                <div className="relative">
                  <select
                    value={breakMinutes}
                    onChange={(e) => setBreakMinutes(Number(e.target.value))}
                    disabled={isSaving}
                    className="w-full appearance-none px-4 py-3 pr-10 rounded-xl bg-background border-2 border-border transition-all focus:outline-none focus:border-primary focus:ring-4 focus:ring-primary/10 disabled:opacity-50 cursor-pointer"
                  >
                    {breakOptions.map((mins) => (
                      <option key={mins} value={mins}>
                        {formatBreakLabel(mins)}
                      </option>
                    ))}
                  </select>
              ...
              <table className="w-full text-left text-sm whitespace-nowrap">
                <thead className="bg-muted/30 text-muted-foreground">
                  <tr>
                    ...
                    <th className="px-6 py-4 font-medium text-right">休憩</th>
                    <th className="px-6 py-4 font-medium text-right">実労働</th>
                    <th className="px-6 py-4 font-medium text-right">過不足</th>
                    <th className="px-6 py-4 font-medium text-center w-24"></th>
                  </tr>
                </thead>
                      ...
                      return (
                        ...
                        >
                          ...
                          <td className="px-6 py-4 text-right text-muted-foreground">
                            {(() => { const bm = log.breakMinutes ?? 60; const h = Math.floor(bm/60); const m = bm%60; return `${h}:${m.toString().padStart(2,'0')}`; })()}
                          </td>
